...
//Rare exceptions to indicate a bug in the _customermoduletest code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_customermoduletest = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_customermoduletest]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.customermoduletest.module"
